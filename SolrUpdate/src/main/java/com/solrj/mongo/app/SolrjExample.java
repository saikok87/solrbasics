...
public class SolrjExample {
	...
	String solrURL = "http://localhost:8983/solr/srchmoviescollection";
	...
	private void execute() {
		...
		SolrDao<Movies> solrDao = new SolrDao<Movies> (solrURL);
	...
	private void readMongoAndIndexSolr(SolrDao<Movies> solrDao) {
		try {
			MongoClient client = new MongoClient("localhost",27017);
			
			DB db = client.getDB("movies");
			
			DBCollection table = db.getCollection("comedy");
			...
			while(cursor.hasNext()) {
				...
				movies.add(new Movies(record.get("_id").toString(), record.get("name").toString(), record.get("year").toString()));
